...
void AsyncElegantOtaClass::begin(AsyncWebServer *server, const char* username, const char* password){
    ...
    _server->on("/update", HTTP_GET, [&](AsyncWebServerRequest *request){
        if(_authRequired){
            if(!request->authenticate(_username.c_str(), _password.c_str())){
                return request->requestAuthentication();
            }//send(SPIFFS, "/index.html", "text/html", false);beginResponse(SPIFFS, "/update.html", "text/html", false);
        }
        AsyncWebServerResponse *response = request->beginResponse(SPIFFS, "/update.html", "text/html", false);
    ...
    _server->on("/update", HTTP_POST, [&](AsyncWebServerRequest *request) {
        ...
        request->send(200, "text/html", "<meta http-equiv=\"refresh\" content=\"5\"> Please, wait...");
        ...
        request->send(response);
        // Serial.println("Error:" + String(Update.hasError()));
        restart();
